...
export class CognitoService {
  ...
  constructor(private config: CognitoConfig) {
    this.client = new CognitoIdentityProviderClient({
      ...
      ...(process.env.AWS_LAMBDA_FUNCTION_NAME ? {} : { profile: "J-CAMPOS" })
    });
  }

  /**
   * Create a new user in Cognito with verified email (simplified for invitations)
   */
  ...
  /**
   * Create a new user in Cognito with verified email (detailed version)
   */
